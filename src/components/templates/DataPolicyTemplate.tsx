...
const DataPolicyTemplate: React.FC = () => {
  ...
  useEffect(() => {
    ...
    dataPolicydata.forEach((section) => {
  ...
  const renderContent = (content: ReactNode[]) => {
    return (
      <div className="flex flex-col gap-4 text-[#1F1F1F] leading-[170%] text-sm md:text-base ">
  ...
  return (
    <div className="w-full bg-primary font-mona-sans relative z-0">
      <div className="absolute z-[-1] w-full h-svh bg-linear-to-b from-[#E6E2FC] to-primary"></div>
      {/* Hero Section */}
        ...
        <div className="flex flex-col md:grid md:grid-cols-3 gap-6 md:gap-8 lg:gap-12 items-start">
                ...
                <button
                  ...
                  className={`text-left py-2.5 px-4 rounded-lg text-base transition-all border ${
                    activeSection === item.id
                      ...
                      : "text-[#1F1F1F] hover:bg-[#F5F5F5] border-[#EFEDFC] hover:border-[#E5E5E5]"
                  }`}
                >
                  {item.title}
                </button>
          ...
          <main className="col-span-1 md:col-span-2 p-4 md:p-6 lg:p-10 rounded-2xl md:rounded-3xl lg:rounded-4xl border border-[#D1D1D1]">
            <div className="flex flex-col items-start gap-12.5">
              {dataPolicydata.map((section: DataPolicySection) => (
                <section
                  key={section.id}
                  id={section.id}
...
export default DataPolicyTemplate;
